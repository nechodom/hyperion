...
async fn build_inner(
    ...
    for (idx, h) in ir.hostings.iter().enumerate() {
        ...
        // One `du` per site, taken above so it can also size the tar budget —
        // not a second one here. `unwrap_or(0)` keeps 0 as the "never measured"
        // marker the manifest relies on: `du -sb` never reports 0 for a
        // directory that exists, so a site that could not be measured makes
        // `inflate_bytes` return None and the disk check fails open.
        let docroot_bytes = docroot_bytes.unwrap_or(0);
