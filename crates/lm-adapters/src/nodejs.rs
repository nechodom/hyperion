...
pub fn unit_path(system_user: &str) -> PathBuf {
    PathBuf::from(format!("/etc/systemd/system/lm-app-{system_user}.service"))
}
...
mod tests {
    ...
    fn render_unit_contains_key_directives() {
        ...
        .expect("render");
        assert!(out.contains("Description=lm app: alice.app (Node 20)"));
        ...
        assert!(out.contains("EnvironmentFile=-/etc/linux-manager/secrets/01J7A"));
    }
    ...
    fn unit_path_is_predictable() {
        assert_eq!(
            ...
            "/etc/systemd/system/lm-app-alice_app.service"
        );
    }
