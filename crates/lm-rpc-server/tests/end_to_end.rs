...
//!   `lm` CLI logic → wire frame → server dispatcher → AgentImpl
...
use lm_rpc::wire::{DbCredentials, DeleteOpts, HostingCreateReq, HostingSelector};
use lm_rpc::AgentApi;
use lm_state::db::open_memory;
use lm_types::{CertInfo, DbProvision, HostingDetail, HostingId, HostingState, PhpVersion};
...
async fn start_agent() -> (std::path::PathBuf, tempfile::TempDir) {
    ...
    let adapters: Arc<dyn lm_core::AdapterPort> = Arc::new(StubAdapters::new());
...
async fn e2e_create_then_list_then_get_then_delete() {
    ...
    let resp = lm_rpc_client::call(&path, Request::HostingCreate(req))
        .await
        .expect("create");
...
async fn e2e_get_unknown_returns_not_found() {
    ...
    .expect("call");
    match resp {
        Response::Error(lm_rpc::RpcError::NotFound { kind, .. }) => {
...
async fn e2e_duplicate_domain_is_already_exists() {
    ...
    lm_rpc_client::call(&path, Request::HostingCreate(req.clone()))
        .await
        .expect("first");
    let resp = lm_rpc_client::call(&path, Request::HostingCreate(req))
        .await
        .expect("second");
    match resp {
        Response::Error(lm_rpc::RpcError::AlreadyExists { kind, .. }) => {
            assert_eq!(kind, "hosting");
        }
...
async fn e2e_validation_error_for_bad_domain() {
    ...
    // refusal in lm-validate by feeding it ourselves.
    let bad = Domain::parse("not-a-real-domain");
    assert!(bad.is_err(), "lm-validate refuses");
