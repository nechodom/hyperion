...
use lm_rpc::wire::{DeleteOpts, HostingCreateReq, HostingCreated, HostingSelector};
use lm_types::{DbProvision, HostingDetail, HostingSummary, PhpVersion};
...
pub async fn post_create(
    ...
    let resp = lm_rpc_client::call(&state.agent_socket, Request::HostingCreate(req.clone()))
        .await
        .map_err(AppError::from)?;
    match resp {
        RpcResponse::HostingCreate(created) => {
            // Re-fetch detail for nice display.
            let detail_resp = lm_rpc_client::call(
                &state.agent_socket,
...
pub async fn get_detail(
    ...
    let resp = lm_rpc_client::call(&state.agent_socket, Request::HostingGet(sel)).await?;
    let detail = match resp {
        RpcResponse::HostingGet(d) => d,
        RpcResponse::Error(lm_rpc::RpcError::NotFound { .. }) => return Err(AppError::NotFound),
...
pub async fn post_delete(
    ...
    let resp =
        lm_rpc_client::call(&state.agent_socket, Request::HostingDelete { sel, opts }).await?;
    match resp {
...
fn parse_selector(s: &str) -> Result<HostingSelector, AppError> {
    if s.contains('.') {
        ...
        Ok(HostingSelector::Id(lm_types::HostingId(s.to_string())))
    }
}
...
fn csrf_token_for(state: &SharedState, ctx: &AuthCtx, form_id: &str) -> String {
    let sid = ctx
        ...
        .map(|s| s.sid.clone())
        .unwrap_or_default();
    lm_auth::csrf::mint(state.csrf_key.as_ref(), &sid, form_id, lm_types::now_secs())
}
...
async fn list_hostings(state: &SharedState) -> Result<Vec<HostingSummary>, String> {
    let resp = lm_rpc_client::call(&state.agent_socket, Request::HostingList)
        .await
