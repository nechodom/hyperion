...
#[derive(Parser, Debug)]
#[command(name = "lm", version, about = "linux-manager CLI")]
struct Cli {
    /// Path to lm-agent's Unix socket.
...
async fn call(cli: &Cli) -> anyhow::Result<Response> {
    ...
    Ok(lm_rpc_client::call(&cli.socket, req).await?)
}
...
mod tests {
    ...
    #[test]
    fn cli_parses_info() {
        let cli = Cli::parse_from(["lm", "info"]);
