...
- **Sites:** `db.sq3` `site` (id, domain_name, user=site-user, type, root_directory) + `php_settings`; docroot `/home/<site-user>/htdocs/<domain>/<root>`; multiple domains per site-user. Parse `/etc/nginx/sites-enabled/<domain>.conf` for aliases/redirects/proxy/SSL paths.
