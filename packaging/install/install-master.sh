...
echo "            (or https://<this-server's-IP>:${LISTEN##*:} if that name"
echo "            does not resolve publicly)"
echo ""
echo "  ⚠ https:// is REQUIRED — the panel speaks TLS from first boot, so"
echo "    a plain http:// attempt fails with ERR_INVALID_HTTP_RESPONSE."
echo "    Browsers autocomplete bare host:port as http://, which is the"
echo "    most common way to hit this. Expect a self-signed certificate"
echo "    warning until you set a panel domain in Settings → General."
echo "  CLI:      hctl info"
